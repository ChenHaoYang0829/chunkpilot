...
public class ServerInitializer {
     ...
     * `permissions()` → `PermissionSet.hasPermission(Permission)`。
     * 新模型的判定逻辑**就是**旧的 "等级 >= N" (`LevelBasedPermissionSet.hasPermission`
     * 内部走 `level().isEqualOrHigherThan(required)`, javap 实证) ⇒ 语义等价。
     */
